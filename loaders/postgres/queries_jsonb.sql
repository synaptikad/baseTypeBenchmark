-- Profil JSONB : requêtes Q1..Q8 identiques sauf filtrage JSONB ciblé
...
-- Q4 Inventaire des points et quantités par étage avec filtre JSONB
WITH RECURSIVE floor_spaces AS (
    ...
    SELECT pt.point_id, n.props
    FROM points pt
    JOIN nodes n ON n.id = pt.point_id
    WHERE n.props @> '{"tags": ["quantity:temperature"]}'
), quantities AS (
    SELECT pp.point_id, m.dst_id AS quantity
    FROM point_props pp
    JOIN edges m ON m.src_id = pp.point_id AND m.rel_type = 'MEASURES'
)
SELECT q.quantity, COUNT(*) AS point_count
FROM quantities q
JOIN equipments e ON e.equipment_id = q.point_id
GROUP BY q.quantity
ORDER BY q.quantity;
