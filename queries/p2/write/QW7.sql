...
SET data = jsonb_set(
    data,
    '{capabilities}',
    COALESCE(data->'capabilities', '[]'::jsonb) || jsonb_build_array(%(new_capability)s::text)
)
WHERE id = %(equipment_id)s
  AND node_type = 'Equipment'
  AND NOT (COALESCE(data->'capabilities', '[]'::jsonb) @> jsonb_build_array(%(new_capability)s::text))
RETURNING
    id,
